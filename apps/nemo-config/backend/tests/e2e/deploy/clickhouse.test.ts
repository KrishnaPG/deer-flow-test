...
describe('Deploy ClickHouse - E2E Tests', () => {
  ...
  beforeEach(async () => {
    testServiceId = generateTestServiceId('clickhouse');
  ...
  it('should deploy ClickHouse container successfully', async () => {
    // Get catalog to get ClickHouse template
    const { data: catalogData } = await get('/api/catalog');
    const clickhouseTemplate = catalogData.find((t: any) => t.id === 'clickhouse');
    expect(clickhouseTemplate).toBeDefined();

    // Deploy ClickHouse
    const { data: deployData, status } = await post('/api/deploy', {
      target_host: 'localhost',
      service_id: testServiceId,
      template: clickhouseTemplate,
      env_values: {}, // ClickHouse template doesn't require special env vars by default
    ...
    expect(details.connectionUrl).toContain('localhost:8123');
  });
});
