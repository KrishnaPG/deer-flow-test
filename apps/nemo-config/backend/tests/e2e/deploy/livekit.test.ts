...
describe('Deploy LiveKit - E2E Tests', () => {
  ...
  beforeEach(async () => {
    testServiceId = generateTestServiceId('livekit');
  ...
  it('should deploy LiveKit container successfully', async () => {
    // Get catalog to get LiveKit template
    const { data: catalogData } = await get('/api/catalog');
    const livekitTemplate = catalogData.find((t: any) => t.id === 'livekit');
    expect(livekitTemplate).toBeDefined();

    // Deploy LiveKit
    const { data: deployData, status } = await post('/api/deploy', {
      target_host: 'localhost',
      service_id: testServiceId,
      template: livekitTemplate,
      env_values: {}, // LiveKit template doesn't require special env vars by default
    ...
    expect(details.connectionUrl).toContain('localhost:7880');
  });
});
