...
describe('Deploy NATS - E2E Tests', () => {
  ...
  beforeEach(async () => {
    testServiceId = generateTestServiceId('nats');
  ...
  it('should deploy NATS container successfully', async () => {
    // Get catalog to get NATS template
    const { data: catalogData } = await get('/api/catalog');
    const natsTemplate = catalogData.find((t: any) => t.id === 'nats');
    expect(natsTemplate).toBeDefined();
    ...
    const { data: deployData, status } = await post('/api/deploy', {
      ...
      env_values: {}, // NATS template doesn't require special env vars by default
    ...
    expect(details.connectionUrl).toContain('localhost:4222');
  });
});
