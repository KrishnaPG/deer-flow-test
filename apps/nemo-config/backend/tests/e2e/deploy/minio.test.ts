...
describe('Deploy Minio - E2E Tests', () => {
  ...
  beforeEach(async () => {
    testServiceId = generateTestServiceId('minio');
  ...
  it('should deploy Minio container successfully', async () => {
    // Get catalog to get Minio template
    const { data: catalogData } = await get('/api/catalog');
    const minioTemplate = catalogData.find((t: any) => t.id === 'minio');
    expect(minioTemplate).toBeDefined();

    // Deploy Minio with required environment values
    ...
    const consulVerified = await verifyConsulConfig(testServiceId, ['url', 'access_key', 'secret_key']);
    ...
    expect(details.connectionUrl).toContain('localhost:9000');
    
    // Verify exports are correctly stored
    expect(details.metadata?.connectionUrl).toContain('minioadmin');
  });
});
