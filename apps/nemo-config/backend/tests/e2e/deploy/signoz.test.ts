...
describe('Deploy Signoz - E2E Tests', () => {
  ...
  beforeEach(async () => {
    testServiceId = generateTestServiceId('signoz');
  ...
  it('should deploy Signoz container successfully', async () => {
    // Get catalog to get Signoz template
    const { data: catalogData } = await get('/api/catalog');
    const signozTemplate = catalogData.find((t: any) => t.id === 'signoz');
    expect(signozTemplate).toBeDefined();

    // Deploy Signoz
    const { data: deployData, status } = await post('/api/deploy', {
      target_host: 'localhost',
      service_id: testServiceId,
      template: signozTemplate,
      env_values: {}, // Signoz template doesn't require special env vars by default
    ...
    const isReady = await waitForContainerStatus(containerName, 'running', 'localhost', 60000); // Signoz might take longer
    ...
    expect(details.connectionUrl).toContain('localhost:3301');
  });
});
