...
## Summary
Provide a backend-hosted Hermes runner service that wraps an in-process Hermes AIAgent. The runner exposes a UI-neutral API that both desktop and web/WASM clients can use, while simultaneously publishing raw, unmapped Hermes session traffic into Redpanda on the `hermes.l0_drop` topic.
...
- A transport-neutral API for starting runs, sending prompts, and subscribing to stream events.
...
- Sending a prompt from a client streams live text and event updates back through the backend API.
