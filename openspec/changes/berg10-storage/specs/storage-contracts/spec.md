...
`physical_location` SHALL NOT be part of the `FileSaved` event; retrieval uses `content_hash` directly.

#### Scenario: FileSaved includes content hash and virtual target
- **WHEN** a blob is successfully stored and registered in the catalog
